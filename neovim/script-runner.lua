...
require("FTerm").setup({
	auto_close = false,
	cmd = (function()
		if vim.fn.findfile("pnpm-lock.yaml") ~= "" then
			return { "zsh", "-c", "pnpm run dev" }
		end

		if vim.fn.findfile("bun.lockb") ~= "" then
			return { "bun", "run", "--silent", "dev" }
		end
...
vim.api.nvim_create_user_command("RUN", function(params)
	...
	local type = params.args:match("(.-):")
	local script = params.args:match(":(.+)")

	-- run the bash script
	if type == "bash" then
		require("FTerm").scratch({ cmd = "bash ./script/" .. script .. ".sh" })
	end
	...
	if type == "node" then
		...
		if vim.fn.findfile("package-lock.json") ~= "" then
			packet_manager = "npm"
		end
	...
	if type == "test" then
		...
		repeat
			if vim.fn.filereadable(root_path .. "/package.json") == 1 then
				...
				if file_path.sub(file_path, 1, 1) == "/" then
